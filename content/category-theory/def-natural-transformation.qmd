...
A **natural transformation** provides a way to transform one [Functor](category-theory/def-functor.qmd) into another while respecting the internal structure of the categories involved.
...
- For each object $X$ in $\mathcal{C}$, a [Morphism](category-theory/def-morphism.qmd) $\eta_X: F(X) \to G(X)$ in $\mathcal{D}$
