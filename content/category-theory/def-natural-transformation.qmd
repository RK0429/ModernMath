...
A **natural transformation** provides a way to transform one @def-functor into another while respecting the internal structure of the categories involved.
...
- For each object $X$ in $\mathcal{C}$, a @def-morphism $\eta_X: F(X) \to G(X)$ in $\mathcal{D}$
