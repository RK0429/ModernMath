...
If $p$ is a [prime number](def-prime.qmd), then for any integer $a$ not divisible by $p$, we have $a^{p-1} \equiv 1 \pmod{p}$ (see [Congruence](def-congruence.qmd)).
...
For $0 < k < p$, the binomial coefficient $\binom{p}{k} = \frac{p!}{k!(p-k)!}$ is divisible by $p$ because $p$ divides the numerator but not the denominator.
...
2. **Example**: Prove that $n^5 - n$ is divisible by $30$ for all integers $n$
   ...
   - Since $n^5 - n = n(n^4 - 1) = n(n^2-1)(n^2+1)$, the divisibility by $2$ and $3$ follows
...
graph TD
    ...
    thm-chinese-remainder --> thm-fermat-little
