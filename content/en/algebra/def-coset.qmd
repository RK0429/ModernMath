...
Every coset of $H$ has the same number of elements as $H$:
$$|gH| = |H| = |Hg|$$
...
- **Parallel subsets**: In abelian groups, cosets are like parallel lines or planes
...
1. **Abelian Groups**: If $G$ is abelian, then $gH = Hg$ for all $g \in G$
...
The concept of cosets is central to [Lagrange's theorem](thm-lagrange.qmd), which states that for a finite group $G$:
$$|G| = |H| \cdot [G:H]$$

### Group Actions

Cosets naturally arise in the study of group actions:
...
Cosets provide a powerful tool for counting in finite groups and proving divisibility results.
...
graph TD
    ...
    def-coset["Definition: Coset"]:::current
