...
where $\Phi(z) = \int_{-\infty}^z \frac{1}{\sqrt{2\pi}} e^{-t^2/2} dt$ is the cumulative distribution function of the standard [normal distribution](def-normal-distribution.qmd).

## Equivalently

The [distribution](def-probability-distribution.qmd) of $\bar{X}_n$ is approximately [normal](def-normal-distribution.qmd) for large $n$:
...
For $n$ fair coin flips (Bernoulli(1/2), see [Binomial Distribution](def-binomial-distribution.qmd)):
- $\mu = 0.5$, $\sigma^2 = 0.25$
...
graph TD
    ...
    click def-normal-distribution "def-normal-distribution.html" "Go to Normal Distribution"
    click def-variance "def-variance.html" "Go to Variance"
```
