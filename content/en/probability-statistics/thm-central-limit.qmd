...
The **Central Limit Theorem (CLT)** is one of the most fundamental results in probability theory, stating that the [distribution](def-probability-distribution.qmd) of the normalized sum of [independent](def-independence.qmd) [random variables](def-random-variable.qmd) converges to a normal distribution.
...
where $\Phi(z) = \int_{-\infty}^z \frac{1}{\sqrt{2\pi}} e^{-t^2/2} dt$ is the cumulative distribution function of the standard normal [distribution](def-probability-distribution.qmd).

## Equivalently

The [distribution](def-probability-distribution.qmd) of $\bar{X}_n$ is approximately normal for large $n$:
