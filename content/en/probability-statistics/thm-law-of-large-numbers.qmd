...
Let $X_1, X_2, \ldots$ be [independent](def-independence.qmd) and identically distributed random variables with finite expected value $\mu = E[X_i]$ and finite [variance](def-variance.qmd) $\sigma^2 = \text{Var}(X_i)$.
...
Since the $X_i$ are [independent](def-independence.qmd):
...
For fair coin flips (Bernoulli(1/2), see [Binomial Distribution](def-binomial-distribution.qmd) for $n=1$), the proportion of heads converges to 1/2 as the number of flips increases.

## Related Concepts

- [Central Limit Theorem](thm-central-limit.qmd): Describes the distribution of the sample mean, not just its convergence
- Concentration inequalities: Provide bounds on convergence rates
