...
- If $n = p$ is @def-prime, then $\phi(p) = p - 1$
- If $n = p^k$, then $\phi(p^k) = p^k - p^{k-1} = p^{k-1}(p - 1)$
- If $\gcd(m, n) = 1$, then $\phi(mn) = \phi(m)\phi(n)$ (multiplicative)

## Proof Using Group Theory

The integers coprime to $n$ form a @def-group under multiplication modulo $n$, denoted $(\mathbb{Z}/n\mathbb{Z})^*$. This group has order $\phi(n)$, so by Lagrange's theorem, any element raised to the group order equals the identity.
