...
- def-event
status: complete
title: 'Theorem: Bayes'' Theorem'
type: Theorem
---

**Bayes' Theorem** provides a way to calculate @def-conditional-probability in the "reverse" direction. It relates $P(A|B)$ to $P(B|A)$, allowing us to update beliefs based on new evidence.

## Statement

For @def-event[s] $A$ and $B$ with $P(A) > 0$ and $P(B) > 0$:
