...
$f: A \to B$ を関数とし、$A$ と $B$ は[濃度](../logic-set-theory/def-cardinality.qmd)が $|A| > |B|$ を満たす有限[集合](../logic-set-theory/def-set.qmd)とします。このとき $f$ は単射ではありません。つまり、$f(a_1) = f(a_2)$ となる異なる要素 $a_1, a_2 \in A$ が存在します。
...
- [包含排除の原理](thm-inclusion-exclusion.qmd)：集合の和集合の大きさを求めるもう一つの基本的な数え上げ原理
- [二項係数](def-binomial-coefficient.qmd)：組み合わせと部分集合の数え上げのため
...
- マウスホイールで**ズーム**イン/アウト
- ノードに**ホバー**して詳細を表示
- [別ウィンドウ](../../output/interactive/ja/thm-pigeonhole-principle.html){target="_blank"}で完全なインタラクティブ版を表示
