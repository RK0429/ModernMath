...
1. **ベルヌーイ分布**: $X \sim \text{Bernoulli}(p)$ ならば、$\text{Var}(X) = p(1-p)$ （$n=1$ の[二項分布](def-binomial-distribution.qmd)を参照）
2. **[二項分布](def-binomial-distribution.qmd)**: $X \sim \text{Binomial}(n, p)$ ならば、$\text{Var}(X) = np(1-p)$
3. **[正規分布](def-normal-distribution.qmd)**: $X \sim \mathcal{N}(\mu, \sigma^2)$ ならば、$\text{Var}(X) = \sigma^2$
...
- [中心極限定理](thm-central-limit.qmd)：標本平均の極限[正規分布](def-normal-distribution.qmd)の分散はσ²/nとなる
