...
$p$ が[素数](def-prime.qmd)で、$a$ が $p$ で割り切れない整数のとき、$a^{p-1} \equiv 1 \pmod{p}$ が成り立つ（[合同式](def-congruence.qmd)を参照）。
...
$0 < k < p$ のとき、二項係数 $\binom{p}{k} = \frac{p!}{k!(p-k)!}$ は $p$ で割り切れる。なぜなら、$p$ は分子を割り切るが分母を割り切らないからである。
...
$p$ を法とする非零剰余類は乗法に関して位数 $p-1$ の群を成す。ラグランジュの定理により、この群の任意の元を群の位数でべき乗すると単位元になる。よって $p$ と互いに素な任意の $a$ に対して $a^{p-1} \equiv 1 \pmod{p}$ が成り立つ。
...
- マウスホイールで**ズーム**イン/アウト
- ノードに**ホバー**して詳細を表示
- [別ウィンドウ](../../output/interactive/thm-fermat-little.html){target="_blank"}で完全なインタラクティブ版を表示
