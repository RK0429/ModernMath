...
function createForceGraph(data, containerId, options = {}) {
  ...
  // Add proof status check mark for completed proofs only
  node.each(function(d) {
    if (d.proof_status && d.proof_status === 'completed') {
      const g = d3.select(this);
      g.append("text")
        .text("✔️")
        .attr("x", 0)
        .attr("y", 4)  // Slight offset to center vertically
        .attr("text-anchor", "middle")
        .attr("font-size", "10px")
        .style("fill", "rgba(255, 255, 255, 0.9)")  // Transparent white
