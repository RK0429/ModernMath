...
CORS(
    app,
    origins=["http://localhost:*", "https://rk0429.github.io", "https://*.github.io"],
...
def get_all_nodes():
    ...
    paginated_query = QUERIES["get_all_nodes"].replace(
        "LIMIT 1000", f"LIMIT {per_page} OFFSET {offset}"
    )
...
def login():
    ...
    if data["username"] == "admin" and data["password"] == os.environ.get(
...
def reload_graph():
    ...
    try:
        ...
        return jsonify({"message": "Graph reload initiated", "user": request.user["user_id"]})
    except Exception as e:
