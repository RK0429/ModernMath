  ...
  try {
    ...
    resultsDiv.innerHTML = `
      <div style="border: 1px solid #f0ad4e; background: #fcf8e3; padding: 1em; border-radius: 4px;">
        <h4 style="margin-top: 0; color: #8a6d3b;">Search Service Unavailable</h4>
        <p style="margin-bottom: 0.5em; color: #8a6d3b;">The search API is not currently available. This typically happens when:</p>
        <ul style="margin: 0.5em 0; padding-left: 1.5em; color: #8a6d3b;">
          <li>The API server is not running (requires local setup)</li>
          <li>You're viewing this page on GitHub Pages (API runs locally only)</li>
        </ul>
        <p style="margin-bottom: 0; color: #8a6d3b;">To use search functionality, please run the API locally or browse content using the navigation menu.</p>
      </div>
    `;
  }
}
