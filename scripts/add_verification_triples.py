#!/usr/bin/env python3
"""
Add isVerifiedBy triples to the main knowledge graph for formally verified nodes.
"""
    ...
    for node_id, node_info in mappings.get("node_to_lean", {}).items():
        if "lean_id" in node_info:
            ...
            if "module_name" in node_info:
                g.add((proof_uri, RDFS.comment, 
                       Literal(f"Implemented in {node_info['module_name']}", lang="en")))
            
            verified_count += 1
            print(f"  ✓ Added verification triple for {node_id} -> {node_info['lean_id']}")
